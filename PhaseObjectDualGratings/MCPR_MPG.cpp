...
//  vector<double> dIfiltered = avgFilter(dI, pxlFilter); // Intensity filtered by pixel
	...
	// Output file
	printVector(dIblank,"dIblank_MPG_S1");
	printVector(dIobj,"dIobj_MPG_S1");
	printVector(dGrid,"dGrid_MPG_S1");
	printVector(objectAll,"objectAll_MPG_S1");
	printVector(phasorG.gAmp,"gAmp_MPG_S1");
	printVector(phasorG.gGrid,"gGrid_MPG_S1");
